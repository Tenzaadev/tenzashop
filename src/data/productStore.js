...
import { subscribeProducts, addProduct as fbAddProduct, updateProduct as fbUpdateProduct, deleteProduct as fbDeleteProduct, updateProductStock as fbUpdateStock, getAllProducts } from '@/lib/firestore'

const PRODUCTS_KEY = 'tenza_products'
...
const productImages = {
  ...
  shorts: ['https://images.unsplash.com/photo-1591195853828-11db59a44f6b?w=400&h=500&fit=crop&q=80'],
  jacket: [
...
/* ── In-memory cache synced via Firestore listener ── */
let cachedProducts = []
...
function startFirestoreSync() {
  ...
  try {
    unsubFirestore = subscribeProducts((products) => {
...
async function ensureInitialized() {
  ...
  initPromise = (async () => {
    ...
    if (saved) {
      try { cachedProducts = JSON.parse(saved) } catch {}
    }
    try {
  ...
  if (cachedProducts.length === 0) {
    try {
      ...
      if (saved) {
        ...
        if (cachedProducts.length === 0) cachedProducts = [...defaultProducts]
      } else {
        cachedProducts = [...defaultProducts]
      }
    } catch {
...
export function getProductsByCategory(category) {
  ...
  if (category === 'limited') return products.filter(p => p.isLimited || p.category === 'limited')
  return products.filter(p => p.category === category)
}

export async function addProduct(product) {
  try {
    const fb = await fbAddProduct(product)
    const products = getProducts()
    products.unshift(fb)
    cachedProducts = products
    localStorage.setItem(PRODUCTS_KEY, JSON.stringify(products))
...
export async function updateProduct(productId, updates) {
  try {
    await fbUpdateProduct(productId, updates)
  } catch {}
  const products = getProducts()
...
export async function deleteProduct(productId) {
  ...
  localStorage.setItem(PRODUCTS_KEY, JSON.stringify(cachedProducts))
  notifyListeners()
  try { window.dispatchEvent(new CustomEvent('products-updated')) } catch {}
}

export async function updateStock(productId, newStock) {
  return updateProduct(productId, { stock: Math.max(0, newStock) })
}
...
export function subscribe(callback) {
  ...
  if (cachedProducts.length > 0) callback(cachedProducts)
  return () => { listeners = listeners.filter(fn => fn !== callback) }
}

export { ensureInitialized as seedFromLocalStorage }
